...
const sendFeedback = async ({ name, phone,email,Itn,file,message,token }: IFeedBackParams): Promise<TFeedBackResponse | IError> => {
  try {
    let url = `${process.env.NEXT_PUBLIC_API_URL}/SendFeedback`
  
   
    const response = await axios.post(url,{name,phone,email,file,Itn,message,token});
